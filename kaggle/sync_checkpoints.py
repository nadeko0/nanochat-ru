...
def sync_once(base_dir, remote, log):
    any_failed = False
...
def main():
    ...
    log(f"sync_checkpoints: base_dir={base_dir} remote={args.remote} interval={args.interval}s once={args.once}")

    if args.once:
        ok = sync_once(base_dir, args.remote, log)
        sys.exit(0 if ok else 1)

    while True:
        sync_once(base_dir, args.remote, log)
