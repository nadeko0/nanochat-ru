...
| [2026-08-14_ru_vocab_sweep_sft_eval.ipynb](2026-08-14_ru_vocab_sweep_sft_eval.ipynb) | `vastai_train_ru.ipynb`, full Phase B pipeline on a rented RTX 5070 Ti: corpus download (FineWeb-2 `rus_Cyrl`) -> both tokenizers (16384/32768) -> both pretrains -> RuBLiMP (sampled) on both base checkpoints -> manual `WINNER_VOCAB=32768` decision -> SFT winner (`saiga_ru`) -> chat test -> `eval_repetition --lang ru` -> full RuBLiMP. Jupyter's own save succeeded this time -- a real notebook artifact, not a console-log fallback | `vocab=16384`: min val_bpb 0.652795, CORE 0.0531, RuBLiMP (sampled) 92.36%. `vocab=32768` **(winner)**: min val_bpb **0.616911**, CORE **0.0630**, RuBLiMP (sampled) **93.19%** -- beats 16384 on every metric, the opposite direction from A9's English result. Winner SFT: 32/32 steps, min val_bpb **0.4785**. `eval_repetition --lang ru`: distinct-1 0.8567, distinct-2 0.9717, **0/30 loops**. Full `eval_rublimp.py` (SFT, 45x1000 pairs): **91.10% overall** (down from base's 93.19%, a real SFT regression). See RESEARCH_LOG.md 2026-08-14 for full breakdown |
